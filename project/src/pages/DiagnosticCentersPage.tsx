...
export default function DiagnosticCentersPage() {
  ...
  const fetchDiagnosticCenters = async () => {
    try {
      ...
      if (data) setDiagnosticCenters(data);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-purple-600 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl font-bold mb-4">Diagnostic Centers</h1>
          <p className="text-xl">Find the best diagnostic centers near you</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {diagnosticCenters.length > 0 ? (
            diagnosticCenters.map((center) => (
              <div key={center.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow p-6 border border-gray-100">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-purple-600 font-medium mb-2">{center.specialization}</p>
                    
                    <div className="flex items-center mb-2">
                      <div className="flex">{renderStars(center.rating)}</div>
                      <span className="ml-2 text-sm text-gray-600">
                        ({RatingsCounter.formatCount(RatingsCounter.getRatingCount(center.id))} reviews)
                      </span>
                    </div>

                    <div className="flex items-center text-gray-600 mb-2">
                      <Clock className="h-4 w-4 mr-2" />
                      <span className="text-sm">{center.experience} years experience</span>
                    </div>
